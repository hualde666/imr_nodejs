...
export const getProductoDescripcion = async (req, res) => {
    var descrip = req.headers.descripcion
    console.log(req.headers.descripcion)
    const pool = await getConnection()

    try {
        // descripcion debe incluir % entre la  descripcion a buscar
        pool
            .request()


            .query(`SELECT ID,CODIGO,DESCRIPCION,DISPONIBLE,PRECIO FROM PRODUCTOS WHERE DESCRIPCION LIKE ${descrip}`,
                function (err, result) {
                    if (err) {
                        console.log(err);
                        return res.status(500).send('Error en el servidor');
